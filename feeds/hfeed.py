    ...
    if is_mf2_h_feed:
        for item in is_mf2_h_feed:
            ...
            for child in item["children"][:5]:
                jf2 = get_feed_item_information(child, url, cursor, hcard, channel_uid, add_to_db)

                results.append(jf2)

    elif is_list_of_h_entries:
        print("entries found: " + str(len(is_list_of_h_entries)))

        for item in is_list_of_h_entries:

            jf2 = get_feed_item_information(item, url, cursor, hcard, channel_uid, add_to_db)

            results.append(jf2)

    return results

def get_feed_item_information(child, url, cursor, hcard, channel_uid, add_to_db):
    jf2 = {
        "type": "entry",
    ...
    if child["properties"].get("summary"):
        jf2["content"] = {
            "text": BeautifulSoup(child["properties"].get("summary")[0], "html.parser").get_text(separator="\n"),
            "html": child["properties"].get("summary")[0]
        }
    elif child["properties"].get("content") and child["properties"].get("content")[0].get("html"):
        jf2["content"] = {
            "text": BeautifulSoup(child["properties"].get("content")[0]["html"], "html.parser").get_text(separator="\n"),
            "html": child["properties"].get("content")[0]["html"]
        }
    elif child["properties"].get("content") and type(child["properties"].get("content")[0]) == str:
        jf2["content"] = {
            "text": BeautifulSoup(child["properties"].get("content")[0], "html.parser").get_text(separator="\n"),
    ...
    if add_to_db == True:
        ...
        cursor.execute("INSERT INTO timeline VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (channel_uid, json.dumps(jf2), jf2["published"], "unread", jf2["url"], ten_random_letters, 0, feed_id, last_id, ))
