...
def search_for_feeds():
    ...
    search_url = "https://indieweb-search.jamesg.blog/results?query=discover {}&format=jf2".format(
        query
    )
    ...
    with connection:
        ...
        if not after and not before:
            item_list = cursor.execute(
                "SELECT * FROM timeline WHERE {} hidden = 0 AND feed_id IN (SELECT id FROM following WHERE muted = 0 AND blocked = 0) ORDER BY date DESC, id DESC LIMIT 21;".format(
                    channel_arg, second_channel_arg
                ),
                channel_tuple,
            ).fetchall()
        elif before and not after:
            item_list = cursor.execute(
                "SELECT * FROM timeline WHERE {} hidden = 0 AND id < ? AND feed_id IN (SELECT id FROM following WHERE muted = 0 AND blocked = 0) ORDER BY date DESC, id DESC LIMIT 21;".format(
                    channel_arg, second_channel_arg
                ),
...
def search_for_content():
    ...
    with connection:
        ...
        result = cursor.execute(
            "SELECT * FROM timeline WHERE jf2 LIKE ? ORDER BY date DESC;",
            (f"%{query}%",),
        ).fetchall()
